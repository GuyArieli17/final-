...
class Indexer:
    ...
    def __init__(self, config, all_terms_dict):
        ...
        avg_ram = (psutil.virtual_memory().available // 10)//(self.thread_pool_size +1)
        path = 'MapReduceData/'
    ...
    def addEntitysToPosting(self, term, tweet_id, quantity):
        ...
        if term.upper() not in self.Entitys.keys() and term.upper() not in tmp_pos.keys() and term.lower() not in map_reduce.meta_data.keys():
