...
class Ranker:
    ...
    @staticmethod
    def expand_qurey(DictFromQuery, association_matrix, posting,map_reduce_ag,map_reduce_hq,map_reduce_rz,map_reduce_other):
        ...
        for term, list_added_word in insert_dic_by_term.items():
            for inner_word in list_added_word:
                ...
                posting[inner_term] = Ranker.find_map_reduce(inner_term,map_reduce_ag,map_reduce_hq,map_reduce_rz,map_reduce_other)
        insert_dic_by_term.clear()


    @staticmethod
    def find_map_reduce(self,term,map_reduce_ag,map_reduce_hq,map_reduce_rz,map_reduce_other,map_reduce_doc):
        if 'a' <= term[0].lower() <= 'g':
            return map_reduce_ag.read_from(term)
        elif 'h' <= term[0].lower() <= 'q':
            return map_reduce_hq.read_from(term)
        elif 'r' <= term[0].lower() <= 'z':
            return map_reduce_rz.read_from(term)
        else:
            return map_reduce_other.read_from(term)



    @staticmethod
    def rank_relevant_doc(relevant_doc,dictFromQuery,posting,map_reduce_ag,map_reduce_hq,map_reduce_rz,map_reduce_other,num_docs_to_retrieve=100):
        """
        ...
        c_matrix.clear()
        Ranker.expand_qurey(dictFromQuery,association_matrix,posting,map_reduce_ag,map_reduce_hq,map_reduce_rz,map_reduce_other)
